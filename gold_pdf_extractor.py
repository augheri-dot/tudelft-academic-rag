...
# Import enrichment & fallback utilities
from gold_enrichment_utils import AbstractReconstructor, HTMLLandingPageParser
...
# ==============================================================================
# MODULE 1: URL Resolution & Metadata Extraction
# ==============================================================================
def extract_best_pdf_url(record: Dict[str, Any]) -> Optional[str]:
    """
    Hierarchical extraction to resolve the highest-priority PDF/OA target URL
    from OpenAlex or TU Delft metadata schema.
    """
    ...
    if isinstance(primary_loc, dict):
        ...
        landing = str(primary_loc.get("landing_page_url", "")).lower()
        if landing.endswith(".pdf"):
            return primary_loc["landing_page_url"]

    best_oa = record.get("best_oa_location") or record.get("open_access") or {}
    if isinstance(best_oa, dict):
        if best_oa.get("pdf_url"):
            return best_oa["pdf_url"]
        oa_url = str(best_oa.get("oa_url", "")).lower()
    ...
    for loc in locations:
        if isinstance(loc, dict):
            if loc.get("pdf_url"):
                return loc["pdf_url"]
            landing = str(loc.get("landing_page_url", "")).lower()
            if landing.endswith(".pdf"):
...
# ==============================================================================
# MODULE 2: Isolated CPU Parser Worker
# ==============================================================================
...
# ==============================================================================
# MODULE 3: Thread-Safe Domain Rate Limiter & Downloader
# ==============================================================================
class BoundedDomainRateLimiter:
    def __init__(self, limit_per_domain: int = 3):
        self.limit_per_domain = limit_per_domain
        self._semaphores: Dict[str, asyncio.Semaphore] = {}
        self._lock = asyncio.Lock()

    async def get_semaphore(self, url: str) -> asyncio.Semaphore:
        domain = urlparse(url).netloc.lower() or "default"
        async with self._lock:
            if domain not in self._semaphores:
                self._semaphores[domain] = asyncio.Semaphore(self.limit_per_domain)
...
async def fetch_pdf_resilient(
    ...
    semaphore = await rate_limiter.get_semaphore(url)
        ...
        for attempt in range(1, max_retries + 1):
            try:
                timeout = aiohttp.ClientTimeout(
                    total=35.0, connect=8.0, sock_connect=8.0, sock_read=15.0
                )
                headers = {
                ...
                async with session.get(current_target_url, timeout=timeout, allow_redirects=True, headers=headers) as resp:
                    ...
                    if resp.status != 200:
                        return None, f"HTTP_{resp.status}", None
                    ...
                    if any(invalid in content_type for invalid in ("text/html", "application/xhtml+xml")):
                        ...
                        if match:
                            ...
                            continue
                        return None, "HTTP_RETURNED_HTML_LANDING_PAGE", html_content

                    buffer = bytearray()
                    async for chunk in resp.content.iter_chunked(STREAM_CHUNK_SIZE_BYTES):
                        buffer.extend(chunk)
                        if len(buffer) > max_file_size_bytes:
                            return None, f"FILE_EXCEEDED_MAX_SIZE_{max_file_size_bytes // (1024*1024)}MB", None

                    if not buffer:
                        return None, "EMPTY_PAYLOAD", None

                    if not buffer.startswith(PDF_MAGIC_BYTES):
                        return None, "INVALID_BINARY_FORMAT", None

                    return bytes(buffer), "HTTP_200", None

            except (aiohttp.ClientError, asyncio.TimeoutError) as err:
                if attempt == max_retries:
                    return None, f"NETWORK_ERROR_{type(err).__name__}", None
                await asyncio.sleep(attempt * 1.5)

        return None, "RETRIES_EXHAUSTED", None


# ==============================================================================
# MODULE 4: Azure Block Blob Streaming Writer with Existing Block Preservation
# ==============================================================================
class AzureBlockBlobWriter:
    ...
    async def initialize_existing_blocks(self) -> None:
        """
        Retrieves committed blocks from the target blob so new staged blocks
        are appended rather than overwriting existing historical data.
        """
        try:
            if await self.blob_client.exists():
                committed_blocks, _ = await self.blob_client.get_block_list(block_list_type="committed")
                self.block_list = [b["id"] for b in committed_blocks]
                logger.info(f"Preserving {len(self.block_list)} existing committed blocks for append strategy.")
        except Exception as err:
            logger.warning(f"Could not fetch existing committed block list: {err}. Starting with fresh block hierarchy.")
    ...
    async def finalize(self) -> None:
        try:
            ...
            if self.block_list:
                logger.info(f"Committing total {len(self.block_list)} blocks to Azure Storage...")
                await self.blob_client.commit_block_list(self.block_list)
                logger.info("Successfully committed block list.")
            else:
                logger.warning("No blocks staged/committed. Output payload empty.")
        except Exception as err:
...
class SmartStreamDecompressor:
    ...
    def decompress_chunk(self, chunk: bytes) -> str:
        ...
        try:
            ...
            logger.warning("Stream is not standard Gzip/Zlib binary. Falling back to raw text decoding.")
            self._is_raw = True
            return chunk.decode("utf-8", errors="ignore")
...
# ==============================================================================
# MODULE 6: Enterprise Pipeline Orchestrator
# ==============================================================================
class GoldPDFExtractionEngine:
    def __init__(self):
        ...
        self.fallback_success_counter = 0
    ...
    async def _load_existing_checkpoint(self, output_blob_client: BlobClient):
        """
        Loads existing processed record IDs from Azure Blob Storage into memory
        to skip reprocessing during execution restarts.
        """
        try:
            if not await output_blob_client.exists():
                logger.info("Output blob absent. Executing initial cold pipeline run.")
                return

            logger.info("Loading checkpoint registry from existing output blob...")
            decompressor = SmartStreamDecompressor()
            text_buffer = []

            download_stream = await output_blob_client.download_blob()
                ...
                for line in lines:
                    ...
                    if line_str:
                        try:
                            ...
                            if rec_id and status in (
                                "SUCCESS",
                                "FALLBACK_ABSTRACT_SUCCESS",
                                "FALLBACK_HTML_ABSTRACT_SUCCESS",
                                "FALLBACK_HTML_BODY_SUCCESS"
                            ):
                                self.processed_ids.add(rec_id)
                        except json.JSONDecodeError:
                            continue

            logger.info(f"Checkpoint registry populated with {len(self.processed_ids)} completed IDs.")
        except Exception as err:
            logger.warning(f"Checkpoint load failed; pipeline resuming without state filter: {err}")

    def _apply_fallback_enrichment(self, record: Dict[str, Any], html_content: Optional[str] = None) -> bool:
        if html_content:
            parsed_html = HTMLLandingPageParser.parse_html(html_content)
            if parsed_html.get("extracted_abstract"):
                record["full_text"] = parsed_html["extracted_abstract"]
                record["extraction_status"] = "FALLBACK_HTML_ABSTRACT_SUCCESS"
                return True
            elif parsed_html.get("html_body_text"):
                record["full_text"] = parsed_html["html_body_text"]
                record["extraction_status"] = "FALLBACK_HTML_BODY_SUCCESS"
                return True

        inverted_index = record.get("abstract_inverted_index")
        reconstructed_abstract = AbstractReconstructor.reconstruct(inverted_index)
        if reconstructed_abstract:
            record["full_text"] = reconstructed_abstract
            record["extraction_status"] = "FALLBACK_ABSTRACT_SUCCESS"
            return True
        ...
        try:
                ...
                for line in lines:
                    ...
                    if line_str:
                        try:
                            ...
                            if rec_id and rec_id in self.processed_ids:
            ...
            if final_text.strip():
                try:
                    ...
                    rec_id = record.get("id")
                    if not (rec_id and rec_id in self.processed_ids):
                        await self.work_queue.put(record)
                        total_records_queued += 1
                    else:
                        self.skipped_counter += 1
            ...
            logger.info(
                f"Reader producer completed. Queued: {total_records_queued}, "
                f"Skipped (Checkpoint): {self.skipped_counter}, Malformed JSON: {self.malformed_json_counter}"
            )
    ...
    async def _async_worker(self, http_session: aiohttp.ClientSession):
        while True:
            ...
            try:
                ...
                if not pdf_url:
                    is_fallback_ok = self._apply_fallback_enrichment(record)
                    if not is_fallback_ok:
                        record["full_text"] = None
                        record["extraction_status"] = "NO_URL"
                    await self.result_queue.put((record, is_fallback_ok, True))
                else:
                    pdf_bytes, http_status, html_content = await fetch_pdf_resilient(
                        http_session, pdf_url, self.rate_limiter
                    )
                    if not pdf_bytes:
                        is_fallback_ok = self._apply_fallback_enrichment(record, html_content)
                        if not is_fallback_ok:
                            record["full_text"] = None
                        ...
                        if parse_status == "SUCCESS":
                            ...
                            await self.result_queue.put((record, True, False))
                        else:
                            is_fallback_ok = self._apply_fallback_enrichment(record)
                            if not is_fallback_ok:
                                record["full_text"] = None
                ...
                await self.result_queue.put((record, False, False))
            finally:
                self.work_queue.task_done()
    ...
    async def _writer_consumer(self, blob_writer: AzureBlockBlobWriter):
        while True:
            ...
            record, is_success, is_fallback = item
            self.processed_counter += 1

            if is_success:
                if is_fallback:
                    self.fallback_success_counter += 1
                else:
            ...
            if self.processed_counter % 500 == 0:
                logger.info(
                    ...
                    f"PDF Success={self.success_counter} | Fallback Success={self.fallback_success_counter} | "
                    f"Failed={self.failed_counter} | Skipped={self.skipped_counter}"
                )
    ...
    async def run(self):
        ...
        await self._load_existing_checkpoint(output_blob)

        writer = AzureBlockBlobWriter(output_blob)
        await writer.initialize_existing_blocks()
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            try:
                ...
                # Signal async workers to terminate
                for _ in range(MAX_CONCURRENT_DOWNLOADS):
                    await self.work_queue.put(None)
                await asyncio.gather(*worker_tasks)

                # Signal writer consumer to terminate
                ...
                logger.info(
                    ...
                    f"Total Processed: {self.processed_counter} (PDF Success: {self.success_counter}, "
                    f"Fallback Success: {self.fallback_success_counter}, Failed: {self.failed_counter}, "
                    f"Skipped Checkpoint: {self.skipped_counter})"
                )
